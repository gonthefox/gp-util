        ...
        (cond ((atom object) acc)
	      ...
	      (cond 
		    ...
		    ((and (eq (dom-tag object) 'div) (string= (dom-attr object 'class) "claim"))      
                                                     (claim-text-renderer-1 (dom-children object) acc ))
		    ((and (eq (dom-tag object) 'div) (string= (dom-attr object 'class) "claim-text")) 
;		                                     (claim-text-renderer-1 (dom-children object) (cons (format "%s\n" (dom-text object)) acc)))
		                                     (claim-text-renderer-1 (dom-children object) (cons (format "%s\n" (dom-texts object)) acc)))
		    ((eq (dom-tag object) 'div)             (claim-text-renderer-1 (dom-children object) acc))
		    (t acc)))))
      ...
      (lambda (acc object) 
        (cond ((atom object) acc)
	      ...
	      (cond 
		    ...
		    ((eq (dom-tag object) 'claim-statement) (claims-renderer-1 (dom-children object) (cons (format "%s\n" (dom-children object)) acc)))
		    ((and (eq (dom-tag object) 'div) (string= (dom-attr object 'class) "claim")) (cons (claim-text-renderer (dom-children object)) acc))
		             
;                                                     (claims-renderer-1 (dom-children object) (cons (format "%s\n" (dom-text object)) acc)))
		    ((eq (dom-tag object) 'div)             (claims-renderer-1 (dom-children object) acc))
		    (t acc)))))
...
(claims-renderer c-list)
;(dom-children (nth 2 (nth 3 sample)))
;(claims-renderer-1 c-list 
;sample

;(nth 0 sample) ;=> section
;(nth 1 sample) ;=> ((itemprop . claims))
;(nth 2 sample) ;=> (h2 nil Claims ((span ((itemprop . count)) 20)))
;(nth 3 sample) ;=> (div ((itemprop . content)) (div ((mxw-id . PCLM59642781) (lang . EN) (load-source . patent-office) (class . claims)) (claim-statement nil The invention claimed is:) (div ((class . claim)) (div ((id . CLM-00001) (num . 1) (class . claim)) (div ((class . claim-text)) 1 A content server for storing and distributing digital content (, comprising:))))))
;(nth 4 sample) ;nil
#+END_SRC
...
(The invention claimed is:)
#+begin_quote
(1. A content server for storing and distributing digital content, comprising:
 a processor; a content database configured to electronically store the digital content; and a DRM packager, implemented at least in part in said processor, for adding a universal DRM layer to the digital content stored at said content database, wherein said universal DRM layer is able to access digital content in conformance with a plurality of different DRM implementations, said digital content being in conformance with one of said plurality of different DRM implementations.
 a processor;
 a content database configured to electronically store the digital content; and
 a DRM packager, implemented at least in part in said processor, for adding a universal DRM layer to the digital content stored at said content database, wherein said universal DRM layer is able to access digital content in conformance with a plurality of different DRM implementations, said digital content being in conformance with one of said plurality of different DRM implementations.
)
#+end_quote
#+begin_quote
(2. The content server of  claim 1  wherein said DRM packager is configured to encrypt the digital content based upon the universal DRM layer.
)
#+end_quote
#+begin_quote
(3. The content server of  claim 1  wherein said DRM packager is configured to tamper-proof or watermark the digital content.
)
#+end_quote
#+begin_quote
(4. The content server of  claim 2  wherein said DRM packager is configured to tamper-proof or watermark the digital content.
)
#+end_quote
#+begin_quote
(5. The content server of  claim 1  wherein the DRM packager distributes the digital content over a computer network.
)
#+end_quote
#+begin_quote
(6. The content server of  claim 5  wherein said computer network is the Internet.
)
#+end_quote
#+begin_quote
(7. The content server of  claim 1  wherein the content database and the DRM packager are in communication over a computer network.
)
#+end_quote
#+begin_quote
(8. The content server of  claim 7  wherein said computer network is the Internet.
)
#+end_quote
#+begin_quote
(9. The content server of  claim 1  wherein the DRM packager further distributes a rights package to the user.
)
#+end_quote
#+begin_quote
(10. The content server of  claim 9  wherein the rights package is bundled with the digital content.
)
#+end_quote
#+begin_quote
(11. The content server of  claim 9  wherein the rights package includes a decryption key.
)
#+end_quote
#+begin_quote
(12. The content server of  claim 10  wherein the rights package includes a decryption key.
)
#+end_quote
#+begin_quote
(13. The content server of  claim 1  wherein the digital content includes text-based information.
)
#+end_quote
#+begin_quote
(14. The content server of  claim 1  wherein the digital content includes audio information.
)
#+end_quote
#+begin_quote
(15. The content server of  claim 1  wherein the digital content include visual information.
)
#+end_quote
#+begin_quote
(16. The content server of  claim 1  wherein the digital content includes audio and visual information.
)
#+end_quote
#+begin_quote
(17. The content server of  claim 1  wherein the step of transmitting the digital content includes the step of transmitting the digital content as streaming media.
)
#+end_quote
#+begin_quote
(18. The content server of  claim 1  wherein the DRM packager is configured, in response to a request from a client to receive the digital content in conformance with a first of the plurality of different DRM implementations, to distribute the digital content together with the universal DRM layer added to the digital content.
)
#+end_quote
#+begin_quote
(19. A content server for storing and distributing digital content, comprising:
 a processor; a content database configured to electronically store the digital content; and a DRM packager, implemented at least in part in said processor, for distributing to a client the digital content from said content database in conformance with a universal DRM implementation that can access digital content in conformance with a plurality of different DRM implementations, wherein said DRM packager is configured to encrypt the digital content, to tamper-proof or watermark the digital content, and to distribute the digital content.
 a processor;
 a content database configured to electronically store the digital content; and
 a DRM packager, implemented at least in part in said processor, for distributing to a client the digital content from said content database in conformance with a universal DRM implementation that can access digital content in conformance with a plurality of different DRM implementations,
 wherein said DRM packager is configured to encrypt the digital content, to tamper-proof or watermark the digital content, and to distribute the digital content.
)
#+end_quote
#+begin_quote
(20. The content server of  claim 19 , wherein the DRM packager bundles a rights package with the digital content prior to transmitting the digital content to the client, and wherein the rights package includes a decryption key for decrypting the digital content.
)
#+end_quote
...
(require 'gp-util)
(setq patent-number "US8683200B2")
(gp-abstract-renderer (gp-get-abstract patent-number))
;(gp-description-renderer (gp-get-description patent-number))
;(gp-claims-renderer (gp-get-claims patent-number))
#+END_SRC
